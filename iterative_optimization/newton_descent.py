...
def newton_descent(f, grad_f, x_init, alpha):
    x = np.array(x_init)
    hessian = hessian_f(*x)
    inv_hessian = np.linalg.inv(hessian)
    p = -1 * (inv_hessian) @ grad_f(*x)
    f_list = [f(*x)]
    x_list = [x]
    dir_list = [p]
...
def main():
    ...
    f_list, x_list, dir_list = newton_descent(f, grad_f, x_init, alpha=ALPHA)
    # fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 9))
    # n = min(20, len(f_list))
    # x, y = zip(*x_list[:n])
    # dx, dy = zip(*dir_list[:n])
    # ax1.plot(np.arange(n), f_list[:n])
    ...
    # fig.suptitle("Newton Descent Using Fixed Step $\\alpha = 0.3$")
    # plt.show()
    ...
    ax2.set_xlim(-1, 4)
    ax2.set_ylim(-1, 4)
    fig.suptitle(f"Newton Descent Using Fixed Step $\\alpha = {ALPHA}$")
    ...
    animation.save("newton_descent.gif", writer="imagemagick")
